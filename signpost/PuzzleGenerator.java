...
class PuzzleGenerator implements PuzzleSource {
     ...
     *  encountered.
     *   // FIXME:
     *   Set nFound to the number of squares in the
    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                PlaceList allsuc = sq.successors();
                for (Place pl: allsuc) {
                    Sq s1 = model.get(pl);
                    if (sq.connectable(s1)) {
                        nFound += 1;
                        found = s1;
                        if (sq.sequenceNum() > 0 && s1.sequenceNum() > 0) {
                            found = s1;
                            nFound = 1;
                            break;
                        }
     ...
     *  encountered.
     *  // FIXME:
     *  Set nFound to the number of squares that are
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                PlaceList allpred = sq.predecessors();
                for (Place pl: allpred) {
                    Sq s0 = model.get(pl);
                    if (s0.connectable(sq)) {
                        nFound += 1;
                        found = s0;
                        if (s0.sequenceNum() > 0 && sq.sequenceNum() > 0) {
                            nFound = 1;
                            found = s0;
                            break;
                        }
