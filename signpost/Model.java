...
class Model implements Iterable<Model.Sq> {
     ...
     *         //        1 - last appear;
     *         else throw IllegalArgumentException (see
     *         //        badArgs utility).
     *          fix
     *          For each Sq object on the board, set its _successors
     *                  and
     *         //      _predecessor lists to the lists of locations of all cells
     *         //        that it might connect to
     *                    (i.e., all cells that are a queen
     *         //        move away in the direction of its arrow,
     *                  and of all cells
    ...
    Model(int[][] solution) {
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                int k = _solution[i][j];
                _solnNumToPlace[k - 1] = Place.pl(i, j);
            }
        }
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                ...
                int dir = 0;
                if (k != last) {
                    dir = _solnNumToPlace[k - 1].dirOf(_solnNumToPlace[k]);
                }
                if (k == 1 || k == last) {
                    fixed = true;
                    sequence = k;
                ...
                _board[i][j] = new Sq(_solnNumToPlace[k - 1].x,
                        _solnNumToPlace[k - 1].y, sequence, fixed, dir, group);
            }
        ...
        for (Sq sq: _allSquares) {
            ...
            if (sq.sequenceNum() == last) {
                sq._successors = null;
            } else {
                for (Place pl: sq._successors) {
                    Sq other = get(pl);
                    if (other._predecessors == null) {
                        other._predecessors = new PlaceList();
                    }
                    other._predecessors.add(sq.pl);
                }
            }
        }
        _unconnected = last - 1;
     ...
     *       // FIXME:
     *       Fill in the _successor, _predecessor, and _head fields of the
     *         //        copied Sq objects.*/
    Model(Model model) {
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                Sq copied = new Sq(model.get(i, j));
                _board[i][j] = copied;
            }
        }
        ...
        for (Sq sq: _allSquares) {
            ...
            Sq original = model.get(sq.pl);
            Sq suc = original.successor();
            if (suc == null) {
                sq._successor = null;
            } else {
                sq._successor = get(original.successor().pl);
            }
            Sq pred = original.predecessor();
            if (pred == null) {
                sq._predecessor = null;
            } else {
                sq._predecessor = get(original.predecessor().pl);
            }
            Sq head = original.head();
            if (head == null) {
                sq._head = null;
            } else {
                sq._head = get(original.head().pl);
            }
        }
    }
    ...
    /** Initialize MODEL to an empty WIDTH x HEIGHT board with a null solution.
      FIXME:
     Initialize _board to contain nulls and clear all objects from
    //        _allSquares.
     // FIXME:
     Initialize _allSuccSquares so that _allSuccSquares[x][y][dir]
    ...
    boolean autoconnect() {
        ...
        for (Sq sq: _allSquares) {
            if (sq.sequenceNum() > 0) {
                for (Sq aq: _allSquares) {
                    if (aq.sequenceNum() > 0) {
                        if (sq.connectable(aq)) {
                            sq.connect(aq);
                            change = true;
                        }
                    }
    ...
    void solve() {
        for (int i = 0; i < _width; i++) {
            ...
            if (k != 1 && k != _width * _height) {
                sq._predecessor = get(solnNumToPlace(k - 2));
                sq._successor = get(solnNumToPlace(k));
                sq._head = get(solnNumToPlace(0));
            } else if (k == 1) {
                sq._predecessor = null;
                sq._head = sq;
                sq._successor = get(solnNumToPlace(1));
            } else {
                sq._successor = null;
                sq._head = get(solnNumToPlace(0));
                sq._predecessor = get(solnNumToPlace(k - 2));
    ...
    final class Sq {
        ...
        boolean connect(Sq s1) {
            ...
            if (this.sequenceNum() > 0 && s1.sequenceNum() == 0) {
                Sq sq = s1;
                while (sq != null) {
                    sq._sequenceNum = sq.predecessor().sequenceNum() + 1;
                    sq = sq.successor();
                }
            } else if (s1._sequenceNum > 0 && this.sequenceNum() == 0) {
                Sq sq = this;
                while (sq != null) {
                    sq._sequenceNum = sq.successor().sequenceNum() - 1;
                    sq = sq.predecessor();
                }
            }
            Sq sq = s1;
            ...
            if (tgroup != 0 && sgroup != 0) {
                int group = joinGroups(tgroup, sgroup);
                _head._group = group;
            } else if (tgroup == 0 & sgroup != 0) {
                _head._group = 0;
                releaseGroup(sgroup);
            } else if (tgroup != 0 && sgroup == 0) {
                _head._group = 0;
         ...
         *        fix: Set the _head of next and all squares
         *         in its group to
         *         next.*/
        void disconnect() {
            Sq next = _successor;
